...
# Apply the same stale-runtime cleanup to every gateway this template
# supervises. Hermes v2026.8.27 refuses `--replace` when a persisted lock cannot
# be attributed to the current container, and named profiles keep their own
# runtime records under profiles/<name>/.
IFS=',' read -ra extra_profiles <<< "${HERMES_EXTRA_PROFILES:-}"
for raw_profile in "${extra_profiles[@]}"; do
  profile="${raw_profile//[[:space:]]/}"
  if [[ ! "$profile" =~ ^[A-Za-z0-9._-]+$ || "$profile" == "." || "$profile" == ".." ]]; then
    [ -z "$profile" ] || echo "Skipping invalid HERMES_EXTRA_PROFILES entry: $profile" >&2
    continue
  fi
  profile_home="/data/.hermes/profiles/$profile"
  rm -f "$profile_home/gateway.pid" "$profile_home/gateway.lock" "$profile_home/gateway.sock"
done
